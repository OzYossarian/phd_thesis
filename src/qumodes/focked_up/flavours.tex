...
In \Cref{ch:floquetifying}, we referenced the PsiQuantum paper TODO that `unified flavours of fault-tolerance' using the ZX-calculus; they took a ZX-diagram representing the surface code as a static stabilizer code, and showed that this was only a few ZX-diagram rewrites away from an interpretation as a dynamic stabilizer code.
...
One can imagine extending this to some algorithm that performs this process on any GKP diagram, in the same way Ben Rodatz specified an algorithm for this process in the qubit case.
...
\teague{Maybe give logical and detecting webs already?}
...
Here $k \in\ZZ$ can be any integer, as per
\begin{align}
...
We can apply the identity rule and the tadpole rule of \Cref{prop:tadpole_rule_fe}, both of which are fault-equivalent \teague{TODO}:
\begin{equation}
    \begin{aligned}
...
Now for the fun step: so long as we ensure that each edge between a red and green spider doesn't swap which spider it's an input of and which it's an output of, we can freely move the spiders around (only connectivity matters!).
Let's do this:
\begin{equation}
%    \begin{aligned}
...
\end{equation}
So this qumode is initialised in the GKP codestate $\ket{0_\square}$, then undergoes a CSUM gate with its neighbour on the left, then on the right, before being measured out in the position basis via a homodyne measurement.
...
We have omitted the web labels, because every highlighted edge is just labelled $k$ for some fixed integer $k \in \mathbb{Z}$.
\begin{equation}
...
\subsection{A measurement-based GKP code}
...
\begin{equation}
    \tikzfig{qumodes/focked_up/flavours/gkp_square_circuit_no_chis}
\end{equation}
We can apply a series of colour change and multiplier rules, each of which we've shown to be fault-equivalent (TODO):
\begin{equation}
    \begin{aligned}
    ...
    \end{aligned}
\end{equation}
We can then apply the tadpole rule of \Cref{prop:tadpole_rule_fe}, and add back in some $\chi_{c_j}$ terms:
\begin{equation}
    \begin{aligned}
...
Viewing time as going upwards rather than left to right, this diagram represents a set of five qumodes, each initialised in the GKP codestate $\ket{+_\square}$, then undergoing a $CZ$ gate with its left and right neighbours to form a `one-dimensional' cluster state, before being measured out in the position basis via a homodyne measurement.
...
We include the web labels explicitly here because the Fourier gate flips some signs -- it's no longer true that all edges are labelled by some fixed $k \in \mathbb{Z}$.
\begin{equation}
    \tikzfig{qumodes/focked_up/flavours/gkp_square_mbqc_web_logical_x}
\end{equation}
\vspace{10pt}
\begin{equation}
    \tikzfig{qumodes/focked_up/flavours/gkp_square_mbqc_web_logical_z}
\end{equation}
\vspace{10pt}
\begin{equation}
    \tikzfig{qumodes/focked_up/flavours/gkp_square_mbqc_web_detector}
\end{equation}