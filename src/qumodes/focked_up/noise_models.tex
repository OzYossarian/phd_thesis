...
We again call errors $F_{j,k}$ that actually appear in one of the probability distributions \textit{atomic errors}.
...
We can take a dual approach that's a little simpler but less accurate; rather than assigning each atomic error $F_{j,k}$ a probability $p_{j,k}$ of occurring, we can assign it a \textit{weight} $w_{j,k} = \wt(F_{j,k})$.
...
We effectively did this without explicitly stating it in \Cref{subsec:distance} for qubits.
If we want to distinguish this weighted approach from the probability-based one, we call this a \textit{weighted noise model}.
...
\begin{equation}\label{eq:weight_general_error_zx_noise}
    \begin{aligned}
...
The intuition behind mixing the 1-norm and 2-norm is to represent noise on each edge being independent, much like the phenomenological noise model in the circuit-based approach assumes each qumode experiences noise independently.
