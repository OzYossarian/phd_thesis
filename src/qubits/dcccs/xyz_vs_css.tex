...
This is exactly what was predicted by \Cref{dif:XYZ_XZ_hyperedges} -- all measurement errors in the $X^1 Y^1 Z^1$ code correspond to degree $> 2$ vertices in the decoding graph, which makes performance under MWPM worse.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\linewidth]{figures/qubits/dcccs/xyz_vs_css/X1Y1Z1_X1Z1_comparison}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\linewidth]{figures/qubits/dcccs/xyz_vs_css/X1Y1Z1_X1Z1_comparison_cln}
...
and additionally show the \textit{degree $> 2$ likelihood} $p_{> 2}$ -- the likelihood of an error in that noise model corresponding to a degree $> 2$ vertex in the decoding graph.
