...
In the `unrepeated' regime, the two DCCCs we will focus on are
...
\end{aligned}
\end{equation}
Once we allow repeated measurements in \Cref{sec:gauge_dcccs}, we will extend our focus to the more general \textit{$X^aY^bZ^c$ honeycomb code} and the \textit{$X^aZ^b$ honeycomb code}, in which each round of measurements are repeated $a$, $b$ or $c$ times respectively.
This whole chapter is concerned with the properties of these two families of codes and their performances under different noise models and decoders.
