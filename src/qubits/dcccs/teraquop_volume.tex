...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\linewidth]{figures/qubits/dcccs/teraquop_volume/teraquop_volume_schematic}
    \caption{
    ...
    \includegraphics[width=0.9\linewidth]{figures/qubits/dcccs/teraquop_volume/X1Y1Z1_X1Z1_teraquop_linefits}
    \caption{
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\linewidth]{figures/qubits/dcccs/teraquop_volume/volume_verification}
    \caption{
...
In the absence of noise, this measurement outcome will be deterministic.
However, in the noisy case, if the net result of applying the decoder's suggested corrections is that a logical error occurs, this can flip the measurement outcome.
...
For example, this tests a part of a \textit{lattice surgery} operation (where the product of stabilizers measured corresponds to a logical Pauli product), or a part of an operation to move a logical operator (where the region of stabilizers is multiplied into the logical operator to move it).
...
By doing this we assume that places where timelike logical $E$ and $M$ errors can occur are roughly equally prevalent in our logical computation.
...
The teraquop volume is a measure of the spacetime cost of a basic unit of quantum computation.
As with all single-number metrics, it has its drawbacks.
...
Consider a circuit component in which logical qubits idle for a long time.
In this scenario, there are no timelike errors to worry about.
Therefore, the logical qubits should be encoded in a circuit with low teraquop \emph{footprint}, and not necessarily low teraqoup \emph{volume}.
