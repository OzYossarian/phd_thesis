...
which we defined in \teague{TODO - define the colour code in the prelims section, as an example of a stabilizer code, since it's mentioned in this section and the last one.}
...
Other systematic ways one might hope to obtain non-drifting boundaries involve changing the measurement schedule.
Suppose a drifting Floquetification has schedule
$\MMM = [\MMM_0, \MMM_1, \ldots, \MMM_{T}]$.
Consider a new Floquet code with schedule
$\MMM = [\MMM_0, \MMM_1, \ldots, \MMM_{T}, \MMM_{T}, \MMM_{T-1}, \ldots, \MMM_0]$.
This new code would drift in one direction in timesteps $0 \leq t < T$,
but then drifts in exactly the opposite direction in timesteps $T \leq t < 2T$,
and so overall stays within a bounded region,
