...
Consider the smallest integer $T$ such that for all $t \geq T$ the codespace dimension $k_t$ is a constant $k$ -- if such a $T$ exists.
If so, then we say the code becomes \textit{established} at time $T$.
In theory, a dynamic code need without an establishment time can still be useful for computation, provided the computation finishes before all the logical qubits being used have been measured out!
But in practice, all known useful dynamic stabilizer codes do become established,
so in the rest of thesis we always assume this property.
...
After establishment at time $T$, we never again measure logical operator of the code (\textbf{Case 3} of the Pauli measurements).
...
\subsection{Induced logical Cliffords}
...
This therefore uniquely (up to global phase) defines a logical Clifford; namely the Clifford $U$ that acts by conjugation on $\ol{\XXX_j}$ and $\ol{\ZZZ_j}$ as $U\ol{\XXX_j} U^\dagger = \phi_{t,t'}(\ol{\XXX_j})$ and $U\ol{\ZZZ_j} U^\dagger = \phi_{t,t'}(\ol{\ZZZ_j})$.
...
If instead $P$ anti-commuted with some at least one stabilizer, we move unitarily from some codespace $\VVV$ to a new one $\VVV'$, as per \Cref{prop:A_m_unitary_up_to_normalisation}, and possibly have to update the representatives of our logical operators, as per Lemma TODO.
...
\begin{example}
    ...
    We can now view this as a dynamic stabilizer code; we define the operation sequence to be:
    \begin{equation}
