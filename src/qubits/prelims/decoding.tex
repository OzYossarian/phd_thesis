...
We then need to find the error that occurred that (it would have violated exactly these detectors and no others) and correct it.

To make life more complicated (perhaps counterintuitively), we don't even actually have to find the \textit{exact} error that occurred!
We said an error $F$ on circuit $C$ is \textit{trivial} iff $C^F \propto C$, i.e.\ if it occurs, it doesn't actually harm our computation.
By definition, trivial errors also don't violate any detectors.
...
These two measurements form a detector $m_a m_b$, where $m_a = (-1)^a$ and $m_b = (-1)^b$.
\begin{equation*}
...
This is labelled $(m_a m_b, e)$, and we know $m_a m_b = 1$ in the absence of any errors, so $e = 1$.
...
We won't give an exact procedure here, but will give a quick example, and refer the reader wanting full details to \Ccite{gidney2021stim,higgott2021pymatching}.
