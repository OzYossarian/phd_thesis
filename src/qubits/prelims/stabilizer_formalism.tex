...
Almost all of this is standard material (e.g.\ see the textbook TODO from the man himself), with the exception of the presentation of the effect of Clifford unitaries and Pauli measurements on the \textit{stabilizer space} $\VVV_\SSS$ and the \textit{logical Pauli group} $\mathcal{N}(\mathcal{S})/\mathcal{S}$, which I feel is always a little neglected in the literature.
...
\begin{theorem}\label{thm:stabilizer_group_update_measurement}
    ...
    Without loss of generality, we can assume $P$ anticommutes just with generator $S_1$ and commutes with the remaining generators.
...
\begin{example}\label{ex:422_stabilizer_group_updates}
    ...
    This is in \textbf{Case 3}: it commutes with all of $\mathcal{S}$ (trivially), but $\pm X_1 X_2 X_3 X_4 \notin \mathcal{S}$.
    ...
    This is \textbf{Case 3}: it commutes with $a_0 X_1 X_2 X_3 X_4$, but $\pm Z_1 Z_2 Z_3 Z_4 \notin \mathcal{S}$.
    ...
    This is \textbf{Case 1}: we have $a_0 X_1 X_2 X_3 X_4 \in \mathcal{S}$, so the measurement outcome $a_1$ is deterministically $a_0$.
    ...
    This is \textbf{Case 1}: we have $b_0 Z_1 Z_2 Z_3 Z_4 \in \mathcal{S}$, so the measurement outcome $b_1$ is deterministically $b_0$.
    The stabilizer group remains:
    \begin{equation}
        \mathcal{S} = \langle a_0 X_1 X_2 X_3 X_4, b_0 Z_1 Z_2 Z_3 Z_4 \rangle.
    \end{equation}

    We can therefore continue measuring $X_1 X_2 X_3 X_4$ and $Z_1 Z_2 Z_3 Z_4$ and we will always be in \textbf{Case 1}; the outcomes $a_j$ and $b_j$ will deterministically be $a_0$ and $b_0$ respectively, and the stabilizer group will be unchanged.
\end{example}
...
\begin{example}\label{ex:bacon_shor_stabilizer_group_updates}
    ...
    Measure $X_3 X_4$.
    This is \textbf{Case 3}: it commutes with $a_0 X_1 X_2$, but $\pm X_3 X_4 \notin \mathcal{S}$.
    Suppose we obtain outcome $b_0 \in \{+1, -1\}$.
    The stabilizer group becomes
    \begin{equation}
    ...
    Measure $Z_1 Z_3$.
    This is \textbf{Case 2}: $Z_1 Z_3$ anticommutes with both $a_0 X_1 X_2$ and $b_0 X_3 X_4$.
    \item The measurement outcome is uniformly random; suppose we obtain $c_0 \in \{+1, -1\}$.
    ...
    Measure $Z_2 Z_4$.
    This is \textbf{Case 3}: $Z_2 Z_4$ commutes with both generators, but $\pm Z_2 Z_4 \notin \mathcal{S}$.
    ...
    This is \textbf{Case 2}: $X_1 X_2$ anticommutes with $c_0 Z_1 Z_3$ and $d_0 Z_2 Z_4$.
    ...
    Now measure $X_3 X_4$.
    This is in \textbf{Case 1}: we have $a_0 b_0 a_1 X_3 X_4 \in \mathcal{S}$, so the outcome $b_1$ is deterministically $a_0 b_0 a_1$.
    The stabilizer group is unchanged.
    ...
    Measure $Z_1 Z_3$.
    This is in \textbf{Case 2}: $Z_1 Z_3$ anticommutes with both $a_1 X_1 X_2$ and $b_1 X_3 X_4$.
    ...
    Now measure $Z_2 Z_4$.
    This is in \textbf{Case 1}: we have $c_0 d_0 c_1 Z_2 Z_4 \in \mathcal{S}$, so the outcome $d_1$ is deterministically $c_0 d_0 c_1$.
...
\begin{example}\label{ex:teleportation_stabilizer_group_updates}
    ...
    This anti-commutes with $Z_2 Z_3$, so we're in \textbf{Case 2}.
    ...
    This is again in \textbf{Case 2} since it anti-commutes with $X_2 X_3$, so the outcome $b$ is random and the updated stabilizer group is
    \begin{equation}
...
\begin{theorem}
    Let $\mathcal{S}$ be a stabilizer group on $n$ physical qubits with $r$ independent generators.
    Then
    \begin{equation}
...
\begin{example}
    Coninuing \Cref{ex:bacon_shor_stabilizer_group}, consider the stabilizer groups $\SSS = \langle X_1 X_2, X_3 X_4, Z_1 Z_2 Z_3 Z_4 \rangle$ and $\SSS' = \langle X_1 X_2 X_3 X_4, Z_1 Z_3, Z_2 Z_4 \rangle$ on four qubits.
...
\teague{Give Pauli group as partitioned by $\NNN(\SSS)$ and then $\SSS$}