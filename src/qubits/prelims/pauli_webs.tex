...
\teague{Change convetion throughout -- have used $(a, b)$ labels in webs to mean $X^bZ^a$, essentially. Clashes with Pauli convention of writing $X^a Z^b$. Change all diagramas!! Sad times.}
...
\begin{definition}\label{defn:highlighting_single_generator}
    A \textit{highlighting} $H$ of a generator $f$ is an assignment of a tuple $(a_j, b_j) \in \mathbb{F}_2^2$ of binary numbers to each edge $e_j$.
    So for a generator with $l+m$ legs total, a highlighting is an element $H \in \mathbb{F}_2^{2(l+m)}$.
    Graphically, we label each wire with its tuple $(a_j, b_j)$ and highlight the edges green and red:
    \begin{equation}
...
We now declare that internal edges are in fact made of two \textit{half-edges}, and boundary edges are all half-edges.
So we write these as $E_{1/2} = E_\circ \sqcup E_\circ \cup E_\boundary$, where we use $\sqcup$ to specifically denote the disjoint product, i.e.\ we take two copies of the internal edges.
...
\begin{definition}\label{defn:highlighting_general}
    A highlighting $H$ of a general diagram $D$ is an assignment of a tuple $(a_j, b_j) \in \mathbb{F}_2^2$ to each half-edge $e_j$.
    In other words, a highlighting on $D$ is an element of the group $H \in \mathbb{F}_2^{|E_{1/2}|}$.
    Graphically:
    \begin{equation}
...
\begin{definition}\label{defn:pauli_web_single_generator}
    A Pauli web $W$ on a generator $f$ is a highlighting on $f$ with the following additional property:
    \begin{equation}
        \tikzfig{qubits/prelims/pauli_webs/highlighting_single_generator}
        ~~
        \begin{matrix}
...
\begin{proposition}\label{prop:pauli_webs_single_generator_form_subgroup}
    ...
    \begin{proof}
        ...
        \begin{equation}
            \tikzfig{qubits/prelims/pauli_webs/highlighting_single_generator_product}
        \end{equation}
        Replacing each highlighted edge with the corresponding spiders, we see that the defining equation does indeed hold:
        \begin{equation}\label{eq:webs_single_generator_form_subgroup}
        \tikzfig{qubits/prelims/pauli_webs/webs_single_generator_form_subgroup}
        \end{equation}
    \end{proof}
\end{proposition}
...
\begin{definition}\label{defn:pauli_web_general}
    A Pauli web $W$ on a general diagram $D$ with edges $E$ is a highlighting $W \in \mathbb{F}_2^{|E_{1/2}|}$ with the following two properties.
...
Note that since labels on pairs of half-edges that make up an internal edge must be identical, we can equivalently view Pauli webs as elements of $\mathbb{F}_2^{|E|}$ rather than $\mathbb{F}_2^{|E_{1/2}|}$.
...
\subsection{Pauli webs on Clifford diagrams}
...
\begin{proposition}\label{prop:set_of_pauli_webs_pauli_spider}
    ...
    \end{equation}
    \begin{proof}
        For the first web, replace the highlighted legs by the corresponding $\pi$-spiders, apply the gate copy rule of \Cref{eq:copy_rules}, and fuse spiders again afterwards to make them disappear:
        \begin{equation}
...
One can again show that by bending legs around so that they're all output legs, any Clifford ZX-diagram is (up to normalisation) a stabilizer state \teague{cite Coco book}.
...
\subsection{The ZX$_{\pi}$ calculus}

TODO? Say that Pauli webs don't care about $\pi$-spiders.
Definition some kind of `$D$ has same QEC properties ass $D'$' relationship iff they're equal in the ZX$_{\pi}$ calculus?
...
We've shown by this point that stabilizing webs correspond to stabilizers $S \in \SSS_D$ up to scalar, under the mapping $W \mapsto Z_1^{a_1} X_1^{b_1} \ldots Z_m^{a_m} X_m^{b_m}$, where $(a_j, b_j)$ is the label in $W$ of the $j$th output edge of $D$.
...
\begin{proposition}\label{prop:stabilizer_webs_iso}
    For a diagram $D$ formed from Clifford unitaries and Pauli measurements, the group $\WWW_\SSS(D)/\WWW_\DDD(D)$ of stabilizing webs modulo detecting webs is isomorphic to the associated stabilizer group $\SSS_D$.
    \begin{proof}
        A full proof is given in Rodatz' paper TODO.
...
\begin{proposition}\label{prop:logical_webs_iso}
    For a diagram $D$ formed from Clifford unitaries and Pauli measurements, the group $\WWW_\LLL \coloneqq \WWW_(D)/\WWW_\SSS^\SSS(D)$ of logical webs is isomorphic to the associated logical Pauli group modulo phases: $(\NNN(\SSS_D)/\pres{i})/(\SSS_D/\pres{i})$.
    \begin{proof}
        Again, a full proof is given in Rodatz' paper TODO.
        ...
        For logical web $W$, we map the coset $\overline{W} = W\WWW_\SSS^\SSS(D)$ to $\ol{Z_1^{a_1} X_1^{b_1} \ldots Z_m^{a_m} X_m^{b_m}} = (Z_1^{a_1} X_1^{b_1} \ldots Z_m^{a_m} X_m^{b_m})\SSS_D$, where $(a_j, b_j)$ is the label in $W$ of the $j$th output edge of $D$:
        \begin{equation}
            \tikzfig{qubits/prelims/pauli_webs/measure_ZZ_twice_web_logical_Z}
            ~~\mapsto~~
...
It remains to discuss costabilizing webs and detecting webs.
...
\begin{lemma}\label{lem:Clifford_unitary_no_stabilizing_web}
    ...
    \begin{proof}
        ...
        So without loss of generality, consider Clifford unitary $U$ on $m$ qubits with non-trivial costabilizing web $W$.
        Let $(a_j, b_j)$ be the label in $W$ of the $j$th input leg of $U$.
        Define $P = Z_1^{a_1} X_1^{b_1} \ldots Z_m^{a_m} X_m^{b_m}$, which by assumption is not $I$.
        ...
        Now, recall two standard results: firstly, for every non-identity Pauli $P$ we can find a Pauli $Q$ that anti-commutes with it, and secondly, conjugating Paulis by Clifford unitaries preserves their (anti)-commutation relations.
...
\begin{lemma}\label{lem:only_costabilizing_web_on_pauli_measurement}
    The projector $\Pi_{cP}$ corresponding to measuring non-identity $P = Z_1^{a_1} X_1^{b_1} \ldots Z_m^{a_m} X_m^{b_m}$ and getting outcome $c$ has a single non-identity costabilizing web, namely $W = (a_1, b_1, \ldots, a_m, b_m)$.
    \begin{proof}
...
that is, the operation occurring at the latest time step $t$ such that the truncated web $W_t$ would be a non-identity stabilizing web.
...
\end{equation}
Since the restriction of the non-truncated web $W$ to this operation is a non-identity costabilizing web, this operation must be a measurement, by \Cref{lem:Clifford_unitary_no_stabilizing_web}.
Therefore, by \Cref{lem:only_costabilizing_web_on_pauli_measurement}, the Pauli being measured must be $P = Z_1^{a_1} X_1^{b_1} \ldots Z_m^{a_m} X_m^{b_m}$, where $(a_j, b_j)$ is the label of output edge $j$ in the stabilizing web $W_t$.
...
\teague{Converse!?!}
Hopefully one can see that the converse -- every measurement of a logical Pauli operator gives rise to a costabilizing web -- is also true, by basically the same reasoning.
Indeed, we get a bijection.

\begin{proposition}
    For a diagram $D$ formed from Clifford unitaries and Pauli measurements, the group $\WWW^\SSS(D)/\WWW_\DDD(D)$ of costabilizing webs modulo detectors is in bijection with the set of all non-trivial logical operators that are measured during the implementation of $D$.
\end{proposition}

\teague{No bijection for detecting webs and detectors?}
...
We will take a very ZX-centric view of errors; in the next section we will take a deeper dive, giving a more traditional definition of an error model, but will also show that this definition is already encompassed by the ZX-centric one we use here.
Since this subsection will again end up being quite verbose, we start with an example.
\teague{TODO}

Diving in, given a ZX-diagram $D$ with edges $E$, we say an \textit{error} (or sometimes \textit{fault}, from which we derive our notation $F$ in what follows) is an element $F \in \PPP_{|E|}$.
...
\begin{equation}
    D = \tikzfig{figures/qubits/prelims/pauli_webs/measure_ZZ_twice_1_outcomes_edges_labelled}
    \qqquad
    D^F = \tikzfig{figures/qubits/prelims/pauli_webs/measure_ZZ_twice_1_outcomes_edges_labelled_error_applied}
\end{equation}
...
\begin{equation}
    \tikzfig{figures/qubits/prelims/pauli_webs/highlighting_eg}
\end{equation}
Firing a generator according to a highlighting $H$ essentially means adding an $I$, $X$, $Y$ or $Z$ operator to the half-edge incident to it, depending on how that edge is highlighted.
For example, on the left below, we fire both generators, while on the right, we fire just the rightmost one:
\begin{equation}
    \tikzfig{figures/qubits/prelims/pauli_webs/highlighting_fg_eg'_both_spiders_fired}
    \qqqqquad
    \tikzfig{figures/qubits/prelims/pauli_webs/highlighting_fg_eg'_2nd_spider_fired}
\end{equation}
Now for the formal definition.
Suppose $D$ is a ZX-diagram with generators $V$ and edges $E$, and $H$ is a highlighting $H \in \mathbb{Z}_2^{|E_{1/2}|}$.
...
\begin{altdefinition}[\Cref{defn:pauli_web_single_generator}]
    ...
    That is, iff $f^{H)} = \lambda_{H|_f} f$ for some non-zero $\lambda_{H|_f} \in \CC$.
    \begin{equation}
        \tikzfig{qubits/prelims/pauli_webs/highlighting_single_generator}
        ~~
        \begin{matrix}
...
Using the above, and recalling (\Cref{defn:pauli_web_general}) that the restriction of $W$ to each generator $v \in V$ must be a Pauli web for that generator, we can see that the result $D^{H}$ of firing all generators according to $W$ is again proportional to $D$.
...
\teague{Maybe easier just to include the correct phases in the firing in the first place -- then can easily see the rule about phases on spiders opposite colour to web highlighting them need to sum to zero mod $2\pi$.}
...
\begin{lemma}\label{lem:firing_detector_web_propto}
    ...
    \begin{proof}
        ...
        \begin{equation}
            \tikzfig{qubits/prelims/pauli_webs/web_fg_fired}
            \quad=\quad
            \lambda_f \lambda_g \tikzfig{qubits/prelims/pauli_webs/web_fg_no_web}
        \end{equation}
        On the other hand the spiders along this internal edge must all cancel out:
        \begin{equation}
            \tikzfig{qubits/prelims/pauli_webs/web_fg_fired}
            \quad=\quad
            \tikzfig{qubits/prelims/pauli_webs/web_fg_fired_internals_cancel}
        \end{equation}
...
Reminder: we can always view a Pauli web $W$ on a diagram $D$ with edges $E$ as an element of $\mathbb{F}_2^{|E|}$ rather than the usual $\mathbb{F}_2^{|E_{1/2}|}$, if we want -- this is fine because half-edges making up internal edges have identical labels.
...
First, we can see exactly which
...
%Since it's the last paragraph of this section, I'll indulge in a needless comment: in reference to the phrase `how much wood could a woodchuck chuck if a woodchuck could chuck wood?', I wanted to title this subsection `exactly which errors can a detecting web detect since a detecting web can detector errors?'.
