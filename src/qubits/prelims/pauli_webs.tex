...
So we write these as $E_{1/2} = E_\circ \sqcup E_\circ \cup E_\boundary$, where we use $\sqcup$ to specifically denote the disjoint union, i.e.\ we take two copies of the internal edges.
...
\begin{definition}\label{defn:highlighting_general}
    ...
    In other words, a highlighting on $D$ is an element of the group $H \in \mathbb{F}_2^{2|E_{1/2}|}$.
    Graphically:
    \begin{equation}
...
\subsection{The $\text{ZX}/\pi$ calculus}

TODO? Say that Pauli webs don't care about $\pi$-spiders.
Definition some kind of `$D$ has same QEC properties as $D'$' relationship iff they're equal in the ZX$_{\pi}$ calculus?
