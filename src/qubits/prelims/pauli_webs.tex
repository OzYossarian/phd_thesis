...
Given a diagram $D$ generators $V$ and a highlighting $H$, we write the diagram with all generators fired according to $H$ as $D^H$.
If only firing a subset of generator $V' \subseteq V$, we write the resulting diagram as $D^{H|_{V'}}$.
...
\begin{proposition}\label{prop:pauli_webs_single_generator_form_subgroup}
    ...
    \begin{proof}
        ...
        \teague{break tikzfig into many, allow page break between them. Doesn't work for aligned inside equation env, need to use align directly.}
...
\begin{proposition}\label{prop:set_of_pauli_webs_clifford_spider}
    ...
    \begin{proof}
        ...
        \teague{Split into multiple tikzfigures, align right}
        \begin{equation}
...
Therefore by \Cref{prop:stabilizer_webs_iso} there's a stabilizing web $W_T$ that corresponds to this, in that the output legs are labelled $(a_j, b_j)$ such that $Z_1^{a_1} X_1^{b_1} \ldots Z_m^{a_m} X_m^{b_m} = \pm P_T$.
(In fact, there's an equivalence class $W_T \cdot \WWW_\DDD(D_T)$).
And by \Cref{lem:only_costabilizing_web_on_pauli_measurement} there's a costabilizing web on the measurement of $P_T$ whose input legs are also labelled $(a_j, b_j)$ such that $Z_1^{a_1} X_1^{b_1} \ldots Z_m^{a_m} X_m^{b_m} = \pm P_T$.
...
Given an error $F = X_1^{a_1}Z_1^{b_1} \ldots X_{|E_{1/2}|}^{a_{|E_{1/2}|}}Z_{|E_{1/2}|}^{b_{|E_{1/2}|}}$ on half-edges, consider the corresponding highlighting $H = (a_1, b_1, \ldots, a_{|E_{1/2}|}, b_{|E_{1/2}|})$.
...
Now consider an error $F = i^k Z_1^{a_1}X_1^{b_1} \ldots Z_{|E|}^{a_{|E|}}X_{|E|}^{b_{|E|}}$ on the edges of $D$.
Since we've defined errors to be Paulis, the detecting web $W$ on $D$ extends to a detecting web $W^F$ on $D^F$.
\teague{Separate result for this somewhere!}
