...
Given $n$ qubits in some state, we first apply an encoding circuit that puts them in a codestate.
Then we repeatedly measure a generating set $\{S_1, \ldots, S_r\}$ of the stabilizer group.
...
Often we might not actually care if we end up in the codespace of $\{S_1, \ldots, S_r\}$ exactly, or just up to signs -- i.e.\ we end up in the codespace of $\{m_1 S_1, \ldots, m_r S_r\}$ for some $m_j$ (though we need to know which $m_j$ they are!).
If this is the case, one possible encoding circuit is just to start measuring the generators $S_j$ -- up to signs measurement outcomes $m_j$, this will project us into the codespace of $\SSS$.
...
We note that the set of all detectors form a group $\DDD$ under this product; if $D = m_1 \ldots m_d$ and $D' = m_1' \ldots m'_{d'}$ are detectors, then by definition in the absence of noise they evaluate to deterministic values $\pm 1$.
...
We will give an exact condition on when an error violates a detector in \Cref{subsec:detecting_webs_detect_errors}.
An error that never violates any detectors is called \textit{undetectable}.
The set of all violated detectors during a given implementation of the code is the \textit{syndrome}.
...
\begin{example}\label{ex:422_stabilizer_code}
    ...
    Viewed as a stabilizer code, this is called the $\qcode{4, 2, 2}$ code (it uses $4$ physical qubits to encode $2$ logical qubits with an \textit{algebraic distance} of $2$; we'll define distance properly later in \Cref{subsec:distance}).
...
\begin{figure}[tbp]
    \centering
    \includegraphics[width=\linewidth]{figures/qubits/prelims/stabilizer_codes/colour_code_stabilizers_logicals}
    \caption{Left: white circles indicate qubits in the support of the two face operators on the face shaded dark pink. Right: four loops are shown: two cyan, two pink, one of each colour being vertical and one horizontal. Each of these loops support a logical $X$ and $Z$ operator, as indicated by the white circles. The two white circles with a double border are the qubits on which logical operators anti-commute.}
    \label{fig:colour_code_stabilizers_logicals}
\end{figure}

\begin{example}\label{ex:colour_code_stabilizer_code}
    Suppose we place a honeycomb lattice on a torus, as in \Cref{fig:colour_code_stabilizers_logicals}.
    ...
    (In fact, any planar graph with these two properties would do  -- it need not be the honeycomb code specifically.)
    ...
    That is, we can arbitrarily pick two colours $\kappa$ and $\kappa'$, and consider `loop' operators as shown in \Cref{fig:colour_code_stabilizers_logicals} for Paulis $X$ and $Z$.
