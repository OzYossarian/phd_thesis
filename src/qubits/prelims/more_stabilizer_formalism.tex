...
Not too much more complicated is \textbf{Case 1} of the Pauli measurements, where $\pm P \in \mathcal{S}$ and $A_m = \Pi_{\pm P}$.
...
In \textbf{Case 2} we measure a $P$ that anti-commutes with exactly one generator $S_1$ of $\SSS$, with random outcome $m$ and map $A_m = \Pi_{mP}$.
...
\begin{proposition}\label{prop:A_m_unitary_up_to_normalisation}
    In \textbf{Case 2}, the restriction of $\sqrt{2} A_m$ to $\VVV_\SSS$ is a unitary $\VVV_\SSS \to \VVV_{\SSS'}$.
    \begin{proof}
...
Finally, in \textbf{Case 3}, we measure a non-trivial logical operator $P \in \NNN(\SSS) - (\pm \SSS)$, getting outcome $m$ and map $A_m = \Pi_{mP}$.
...
\begin{proposition}
    In \textbf{Case 3}, the restriction of $A_m$ to $\VVV_\SSS$ is a projector $\VVV_\SSS \to \VVV_{\SSS'}$
    \begin{proof}
...
\begin{theorem}\label{thm:codespace_maps}
    Given a stabilizer group $\SSS$, the effect of applying a Clifford and \textbf{Cases 1 and 2} of measuring a Pauli on the codespace $\VVV_\SSS$ is that it is mapped unitarily to the new codespace $\VVV_{\SSS'}$.
    In \textbf{Case 3}, it is mapped projectively onto the new codespace $\VVV_{\SSS'}$.
\end{theorem}
...
\begin{example}\label{ex:bacon_shor_codespace_updates}
    ...
    Both of the first two measurements $X_1 X_2$ and $X_3 X_4$ are in Case 3 of \Cref{thm:stabilizer_group_update_measurement}, so the resulting maps are projections that each halve the dimension of the codespace:
    \begin{equation}
        \VVV_{\SSS_{-1}}
        ...
        \VVV_{\SSS_0'}
    \end{equation}
    The next measurement $Z_1 Z_3$ is in \textbf{Case 2}, so maps us unitarily to the next codespace $\SSS_0''$.
    Then the measurement $Z_2 Z_4$ is in \textbf{Case 3}, so another projector is applied, mapping from $\SSS_0''$ to the new codespace $\SSS_0'''$, which is again half the size:
    \begin{equation}
        \VVV_{\SSS_{0}'}
    ...
    Measuring $X_1 X_2$ is this time in \textbf{Case 2}.
    Writing the new stabilizer group as $\SSS_1$, this measurement thus maps us unitarily from $\VVV_{\SSS_0'''}$ to $\VVV_{\SSS_1}$.
    Measuring $X_3 X_4$ is in \textbf{Case 1}, so at the level of the codespace just applies the identity map to $\VVV_{\SSS_1}$.
    The same holds analogously when measuring $Z_1 Z_3$ and $Z_2 Z_4$; the first of these is in \textbf{Case 2} and maps unitarily from $\VVV_{\SSS_1}$ to $\VVV_{\SSS_1'}$, then the second is in \textbf{Case 1} and applies the identity to $\VVV_{\SSS_1'}$.
...
\begin{example}\label{ex:teleportation_codespace_updates}
    ...
    Both of these were \textbf{Case 2} measurements, so together they unitarily map us from $\VVV_\SSS$ to $\VVV_{\SSS''}$:
    \begin{equation}
        \VVV_{\SSS}
    ...
    Since this is particular a Clifford unitary, this too applies a unitary to our codespace:
    \begin{equation}
        \VVV_{\SSS''}
...
\begin{theorem}\label{thm:logical_pauli_group_update_measurement}
    ...
    \textbf{Case 1:} $\pm P \in \mathcal{S}$.
    ...
    \textbf{Case 2:} $P$ anticommutes with at least one element of $\mathcal{S}$.
    ...
    Also without loss of generality, assume $P$ commutes with all representatives $\XXX_j$ and $\ZZZ_j$.
    ...
    \textbf{Case 3:} $P$ commutes with all of $\mathcal{S}$, but $\pm P \notin \mathcal{S}$.
    In this case, $P$ is in $\NNN(\SSS) - (\pm \SSS)$, so is a non-trivial element of the logical Pauli group $\mathcal{N}(\mathcal{S})/\mathcal{S}$.
    Without loss of generality, we can in fact assume $P = \pm \ZZZ_j$ or $\pm \XXX_j$ for some $1 \leq j \leq k$.
...
Note that in \textbf{Case 2}, the new logical Pauli group might look identical to the old one, but it isn't!
...
Case 3 is the only one in which the induced homomorphism isn't an isomorphism, since the new logical Pauli group is smaller than the original.
In both Cases 2 and 3, we said that without loss of generality we could make assumptions about the commutativity relations between $P$ and representatives $\XXX_j$ and $\ZZZ_j$, which we now spell out.

\teague{TODO: WLOG lemmas}
...
\begin{example}\label{ex:bacon_shor_logical_pauli_group_updates}
    ...
    \begin{equation}
        ...
        \rangle
    \end{equation}
    The first measurement $X_1 X_2$ is in \textbf{Case 3}.
    ...
    Following Lemma TODO, we can write
    \begin{equation}
        \NNN(\SSS_{-1})/\SSS_{-1} = \langle
        \overline{i},
    ...
    Next, we measure $X_3 X_4$, which is in \textbf{Case 3}.
    ...
    Next, we measure $Z_1 Z_3$, which is our first \textbf{Case 2} measurement.
    ...
    The final measurement in round $0$ is $Z_2 Z_4$, which is in \textbf{Case 3}.
    ...
    So first apply Lemma TODO to get:
    \begin{equation}
        \NNN(\SSS_{0}'')/\SSS_{0}'' = \langle
        \overline{i},
    ...
    This is because future measurements are always either in \textbf{Case 1} or \textbf{Case 2}, and the representatives above commute with all of $X_1 X_2$, $X_3 X_4$, $Z_1 Z_3$ and $Z_2 Z_4$.
\end{example}
...
\begin{example}\label{ex:teleportation_logical_pauli_group_updates}
    ...
    This is a Case 2 measurement, so to apply \Cref{thm:logical_pauli_group_update_measurement} we need a new presentation whose representatives commute with $X_1 X_2$.
    ...
    Next we measure $Z_1 Z_2$ and are in a symmetric situation; this is in Case 2, so we get random outcome $b$, and since $X_2 X_3$ is a stabilizer a suitable new presentation for the logical Pauli group is:
    \begin{equation}
        \NNN(\SSS')/\SSS' = \langle
        \overline{i},
...
So, first assume the homomorphism is an isomorphism, which occurs iff all operations are Clifford unitaries, or Case 1 or 2 Pauli measurements.
...
It is then a standard result (e.g.\ Gottesman TODO) that this map $A_m \Pi_{\SSS}$ must be a uniquely determined logical Clifford (unique up to global phase, at least).
...
Hopefully it's clear that in \textbf{Case 1} there's really nothing to prove!
But \textbf{Case 2} and \textbf{Case 3} are non-trivial.

\begin{proposition}
    In \textbf{Case 2}, the induced homomorphism $\phi$ satisfies \Cref{eq:induced_homomorphism_iff_relation}.
...
\begin{proposition}
    In \textbf{Case 3}, the induced homomorphism $\phi$ satisfies \Cref{eq:induced_homomorphism_iff_relation}.
    That is, after measuring $P$ and getting outcome $m$, we have $\phi(\overline{L'}) = \overline{L} \iff (L' A_m)|_{\VVV_\SSS} = (A_m L)|_{\VVV_\SSS}$, where $A_m = \Pi_{mP}$.
    \begin{proof}
...
\begin{lemma}\label{lem:if_in_im_phi_then_commutes_with_P}
    In \textbf{Case 3}, if $\overline{L} \in im(\phi)$ then $L$ commutes with $P$.
    \begin{proof}
